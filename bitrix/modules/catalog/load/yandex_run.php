...
/*					$dbVAT = CCatalogProduct::GetVATInfo($arAcc['ID']);
			...
			foreach ($arYandexFields as $key)
			{
				...
				case 'name':
					...
					$strTmpOff .= "<name>".yandex_text2xml($arAcc["NAME"])."</name>\n";
					break;
				case 'description':
					$strTmpOff .=
						"<description>".
				...
				case 'title':
					if (!is_array($XML_DATA) || !is_array($XML_DATA['XML_DATA']) || !$XML_DATA['XML_DATA'][$key])
					{
						...
						$strTmpOff.= "<".$key.">".yandex_text2xml($arAcc["NAME"])."</".$key.">\n";
					}
					else
					{
						$strValue = '';
				...
				if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
				{
					...
					foreach ($arYandexFields as $key)
					{
						...
						case 'name':
							...
							$strOfferYandex .= "<name>".yandex_text2xml($arOfferItem["NAME"])."</name>\n";
							break;
						case 'description':
						...
						case 'title':
							if (!is_array($XML_DATA) || !is_array($XML_DATA['XML_DATA']) || !$XML_DATA['XML_DATA'][$key])
							{
								...
								$strOfferYandex .= "<".$key.">".yandex_text2xml($arOfferItem["NAME"])."</".$key.">\n";
							}
							else
							{
								$strValue = '';
...
/*						$dbVAT = CCatalogProduct::GetVATInfo($arItem['ID']);
				...
				foreach ($arYandexFields as $key)
				{
					...
					case 'name':
						...
						$strValue = "<name>".yandex_text2xml($arItem["NAME"])."</name>\n";
						break;
					case 'description':
						$strValue =
							"<description>".
					...
					case 'title':
						if (!is_array($XML_DATA) || !is_array($XML_DATA['XML_DATA']) || !$XML_DATA['XML_DATA'][$key])
						{
							...
							$strValue = "<".$key.">".yandex_text2xml($arItem["NAME"])."</".$key.">\n";
						}
						else
						{
