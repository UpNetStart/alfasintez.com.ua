...
$PROD_OPACITY_PRICE = ($arResult['PROPERTIES']['PR_PRICE_OPACITY']['VALUE'] != "") ? $arResult['PROPERTIES']['PR_PRICE_OPACITY']['VALUE']:$arResult['PROPERTIES']['PR_PRICE_OPACITY']['DEFAULT_VALUE'];
$PROD_COLOR_PRICE = ($arResult['PROPERTIES']['PR_PRICE_COLOR']['VALUE'] != "") ? $arResult['PROPERTIES']['PR_PRICE_COLOR']['VALUE']:$arResult['PROPERTIES']['PR_PRICE_COLOR']['DEFAULT_VALUE'];
$PROD_WEIGHT_PRICE = ($arResult['PROPERTIES']['PR_PRICE_WEIGHT']['VALUE'] != "") ? $arResult['PROPERTIES']['PR_PRICE_WEIGHT']['VALUE']:$arResult['PROPERTIES']['PR_PRICE_WEIGHT']['DEFAULT_VALUE'];

$PROD_WEIGHT = $arResult['CATALOG_WEIGHT'];
$PROD_COUNT_BOX = ($arResult['PROPERTIES']['PR_IN_THE_BOX']['VALUE'] != "")?$arResult['PROPERTIES']['PR_IN_THE_BOX']['VALUE']:$arResult['PROPERTIES']['PR_IN_THE_BOX']['DEFAULT_VALUE'];

if($PROD_WEIGHT < 10){
	$PROD_PRICE = $PROD_WEIGHT_PRICE;
}else{
	$PROD_PRICE = $PROD_COLOR_PRICE;
}

if(isset($PROD_PRICE) && isset($PROD_WEIGHT) && isset($PROD_COUNT_BOX) && ($PROD_COUNT_BOX*$PROD_WEIGHT*$PROD_PRICE) != 0) {


	$onePositionPrice = ($PROD_PRICE / 1000) * $PROD_WEIGHT;
	$reloadPrice = $PROD_COUNT_BOX * (($PROD_PRICE / 1000) * $PROD_WEIGHT);// [CATALOG_WEIGHT] [CATALOG_PRICE_1] ([PROPERTIES] [PR_OPACITY_PRICE] [PR_COLOR_PRICE])   ([PROPERTIES] [PR_IN_THE_BOX])
...
$arLinkItem = $arResult['DISPLAY_PROPERTIES']['PR_FILTERS']['LINK_ELEMENT_VALUE'];
?>
...
/*if (isset($arResult['OFFERS']) && !empty($arResult['OFFERS']) && !empty($arResult['OFFERS_PROP']))
	...
	?>
		...
		<div class="for-using clearfix">
			<span class="header-prams" style="text-align: center"> Рекомендуем для </span>
		...
		<div class="item_price" title="Цена за единицу.">
			<? $boolDiscountShow = (0 < $arResult['MIN_PRICE']['DISCOUNT_DIFF']); ?>
			<div class="item_current_price" id="<? echo $arItemIDs['PRICE']; ?>">

				<?if($onePositionPrice != 0){?>
				<span class="price-header"> Стоимость единицы: <? echo round($onePositionPrice, 2)." грн."; ?> </span>
			<?
				}else{
					?>
					<div class="not-avalible">
						Нет в наличии.
					</div>
				<? } ?>
			</div>
